...
class Format():
    ...
    def underline(self,*args):
        try:
            current_tags=self.text.tag_names("sel.first")
            if "undrline" in current_tags:
                self.text.tag_remove("underline","sel.firt""sel.last")
            else:
                self.text.tag_add("underline","sel.firt","sel.last")
...
def main(root,text,menubar):
    ...
    formatMenu = Menu(menubar,bg="RoyalBlue2",fg="white",tearoff=False)
